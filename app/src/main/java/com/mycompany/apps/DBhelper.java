...
//*******************!!!!!! SYSTEM CURRENTTIME IN MILISECOND CODE USED IN SALESRE!!!!**************
	...
	//************************!!!!!UPDATE LOCAL VENDOR!!!!!!!*****************************************
	public boolean updatelocalVendor(String LOCALVENDORID, String LOCALVENDORNAME, String LOCALVENDORCONTACT, String LOCALVENDORADDRESS, String LOCALVENDORCITY, String LOCALVENDORCOUNTRY, String LOCALVENDORZIP, String LOCALVENDORTELE, String LOCALVENDORMOBILE, String LOCALVENDORACTIVE) {
		SQLiteDatabase db = this.getWritableDatabase();
		ContentValues contentValues = new ContentValues();
		...
		contentValues.put("Vend_Inv", LOCALVENDORMOBILE);
...
*/
	...
	public ArrayList<Sales> getProductNamedata(String idorName) {
		...
		try {
			...
			params[0] = idorName + "%";
			params[1] = idorName + "%";
			Cursor productcursor = db.rawQuery("select distinct Prod_Nm,Prod_Id,Batch_No,Exp_Date,MRP,Amount,uom,S_Price,Qty,BarCode,Conv_Fact  from retail_str_stock_master where "
		...
		try {
			...
			if (productcursor.moveToFirst()) {
				do {
					...
					pm.setConversion(productcursor.getString(productcursor.getColumnIndex(PRODUCTCONVERSION)));
	...
	public boolean updateStockQty(ArrayList<Sales> salesBillItems) {
		...
		for (Sales sales : salesBillItems) {
			...
			if (saleQuantity <= stockQuantity) {
				...
				contentValues.put("Qty", Double.toString(newstock));
   ...
   /* String []params=new String[2];
	...
	public void linediscountitem(ArrayList<line_item_Product_Model> list) {
		...
		for (line_item_Product_Model prod : list) {
			...
			values.put("Line_Item_Id", prod.getProductId());
			//   values.put(";Prod_Id",prod.getProductId());
			// Inserting Row
...
/////////////////////////////////////////////////////get all data from retail_str_po_detail ////////////////////////////////////////////////////////////////////////////
	...
	public ArrayList<Sales> getallholdinvoicedata(String data) {
		...
		String[] params = new String[1];
		params[0] = data + "%";
		//Cursor productcursor = db.rawQuery("select a.Tri_Id,a.Prod_Nm,a.Batch_No,a.Exp_Date,a.S_Price,a.Qty,a.MRP,a.UOM,a.Stock,a.Total from tmp_retail_str_sales_detail a,tmp_retail_str_sales_master b where a.Tri_Id = b.Tri_Id and a.Tri_Id Like ? ",params);
	...
	public ArrayList<String> getTransidofholdbill() {
		ArrayList<String> imeilist = new ArrayList<String>();
		SQLiteDatabase db = this.getReadableDatabase();
		Cursor cursor = db.rawQuery("select  Tri_Id from tmp_retail_str_sales_master", null);
		if (cursor.moveToFirst()) {
			do {
...
*/
	...
	public ArrayList<PurchaseProductModelwithpo> getholddataforinventory(String PO) {
		...
		try {
			...
			Cursor productcursor = db.rawQuery(" select Po_No,Grn_Id,Vend_Nm, Prod_Id,Batch_No,Exp_Date, MRP,S_Price ,Prod_Nm ,P_Price, Qty, Amount,Profit_Margin, Uom,Conv_Fact,Ind_Nm from retail_str_stock_master_hold where "
							+ " Grn_Id  like ? "
					, params);
...
public ArrayList<String> getBillLevel() {
	...
	public ArrayList<localvendor> getAlllocalvendorforserverupload() {

		ArrayList<localvendor> localvendorlistforserver = new ArrayList<localvendor>();
		try {

			SQLiteDatabase db = this.getReadableDatabase();
			Cursor res = db.rawQuery("select * from retail_store_vendor where S_Flag like '0%' ", null);
			if (res.moveToFirst()) {
				do {
					localvendor localvendor = new localvendor();
					localvendor.setLocalvendorid(res.getString(res.getColumnIndex(LOCALVENDORID)));
					...
					localvendor.setLocalvendorzip(res.getString(res.getColumnIndex(LOCALVENDORZIP)));
					localvendor.setLocalactive(res.getString(res.getColumnIndex(LOCALVENDORACTIVE)));
					localvendorlistforserver.add(localvendor);
				}
				while (res.moveToNext());
			}
		...
		return localvendorlistforserver;
	}

	public void tempsavesalesmaster(String valuetrans, String grandtotal) {
		...
		PersistenceManager.getStoreId(mycontext);
		contentValues.put("Store_Id", PersistenceManager.getStoreId(mycontext));
		contentValues.put("Tri_Id", valuetrans);
		contentValues.put("Grand_Total",grandtotal);
		db.insert("tmp_retail_str_sales_master", null, contentValues);
